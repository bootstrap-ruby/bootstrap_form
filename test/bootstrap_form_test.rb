...
class BootstrapFormTest < ActionView::TestCase
  ...
  test "default-style forms" do
    expected = %{<form accept-charset="UTF-8" action="/users" class="new_user" id="new_user" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /></form>}
    assert_dom_equal expected, bootstrap_form_for(@user) { |f| nil }
  end

  test "inline-style forms" do
    expected = %{<form accept-charset="UTF-8" action="/users" class="form-inline" id="new_user" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /></form>}
    assert_dom_equal expected, bootstrap_form_for(@user, layout: :inline) { |f| nil }
  end
  ...
  test "horizontal-style forms" do
    ...
    assert_dom_equal expected, bootstrap_form_for(@user, layout: :horizontal) { |f| f.email_field :email }
  end
  ...
  test "existing styles aren't clobbered when specifying a form style" do
    ...
    assert_dom_equal expected, bootstrap_form_for(@user, layout: :horizontal, html: { class: "my-style" }) { |f| f.email_field :email }
  end

  test "given role attribute should not be covered by default role attribute" do
    expected = %{<form accept-charset="UTF-8" action="/users" class="new_user" id="new_user" method="post" role="not-a-form"><input name="utf8" type="hidden" value="&#x2713;" /></form>}
    assert_dom_equal expected, bootstrap_form_for(@user, html: { role: 'not-a-form'}) {|f| nil}
  end

  test "bootstrap_form_tag acts like a form tag" do
    expected = %{<form accept-charset="UTF-8" action="/users" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /><div class="form-group"><label class="control-label" for="email">Your Email</label><input class="form-control" id="email" name="email" type="text" /></div></form>}
    assert_dom_equal expected, bootstrap_form_tag(url: '/users') { |f| f.text_field :email, label: "Your Email" }
  end

  test "bootstrap_form_tag does not clobber custom options" do
    expected = %{<form accept-charset="UTF-8" action="/users" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /><div class="form-group"><label class="control-label" for="ID">Email</label><input class="form-control" id="ID" name="NAME" type="text" /></div></form>}
    assert_dom_equal expected, bootstrap_form_tag(url: '/users') { |f| f.text_field :email, name: 'NAME', id: "ID" }
  end

  test "bootstrap_form_tag allows an empty name for checkboxes" do
    expected = %{<form accept-charset="UTF-8" action="/users" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /><div class="checkbox"><label for="_misc"><input name="[misc]" type="hidden" value="0" /><input id="_misc" name="[misc]" type="checkbox" value="1" /> Misc</label></div></form>}
    assert_dom_equal expected, bootstrap_form_tag(url: '/users') { |f| f.check_box :misc }
  end
  ...
  test "errors display correctly and inline_errors can also be on when label_errors is true" do
    ...
    assert_dom_equal expected, bootstrap_form_for(@user, label_errors: true, inline_errors: true) { |f| f.text_field :email }
  end

  test "label error messages use humanized attribute names" do
    I18n.backend.store_translations(:en, {activerecord: {attributes: {user: {email: 'Email'}}}})
  ...
  test "alert message is wrapped correctly" do
    ...
    assert_dom_equal expected, @builder.alert_message('Please fix the following errors:')
  end
  ...
  test "changing the class name for the alert message" do
    ...
    assert_dom_equal expected, @builder.alert_message('Please fix the following errors:', class: 'my-css-class')
  end
  ...
  test "alert_message allows the error_summary to be turned off" do
    ...
    expected = %{<form accept-charset="UTF-8" action="/users" class="new_user" id="new_user" method="post" role="form"><input name="utf8" type="hidden" value="&#x2713;" /><div class="alert alert-danger"><p>Please fix the following errors:</p></div></form>}
    #assert_dom_equal expected, output
  end
  ...
  test "error_summary returns an unordered list of errors" do
    ...
    assert_dom_equal expected, @builder.error_summary
  end
  ...
  test 'errors_on renders the errors for a specific attribute when invalid' do
    ...
    assert_dom_equal expected, @builder.errors_on(:email)
  end
  ...
  test "custom label width for horizontal forms" do
    ...
    assert_dom_equal expected, bootstrap_form_for(@user, layout: :horizontal) { |f| f.email_field :email, label_col: 'col-sm-1' }
  end
  ...
  test "custom input width for horizontal forms" do
    ...
    assert_dom_equal expected, bootstrap_form_for(@user, layout: :horizontal) { |f| f.email_field :email, control_col: 'col-sm-5' }
  end
  ...
  test "allows the form object to be nil" do
    ...
    assert_dom_equal expected, builder.text_field(:email)
  end
  ...
  test 'errors_on hide attribute name in message' do
    ...
    assert_dom_equal expected, @builder.errors_on(:email, hide_attribute_name: true)
  end
end
